...
describe('FilmInfo.filmInfoNote() test', () => {
  it('should generate film note citation format', () => {
    expect(filmInfoNote(filmInfo)).toBe('(Original Release Date; City: Studio/Distributor, Video Release Date), ');
  });

  it('should generate note citation format with no original release specified', () => {
    expect(filmInfoNote(noOriginalRelease)).toBe('(City: Studio/Distributor, Video Release Date), ');
  });

  it('should generate note citation format with no city specified', () => {
    expect(filmInfoNote(noCity)).toBe('(Original Release Date; Studio/Distributor, Video Release Date), ');
  });

  it('should generate note citation format with no studio/distributor specified', () => {
    expect(filmInfoNote(noDistributor)).toBe('(Original Release Date; City: Video Release Date), ');
  });

  it('should generate note citation format with no video release date specified', () => {
    expect(filmInfoNote(noVideoRelease)).toBe('(Original Release Date; City: Studio/Distributor), ');
  });
  ...
  it('should generate note citation format with only original release date and city', () => {
    expect(filmInfoNote(onlyOriginalReleaseAndCity)).toBe('(Original Release Date; City), ');
  });

  it('should generate note citation format with only distributor and video release date', () => {
    expect(filmInfoNote(onlyDistributorAndVideoRelease)).toBe('(Studio/Distributor, Video Release Date), ');
  });

  it('should generate note citation format with only original release date and distributor specified ', () => {
    expect(filmInfoNote(onlyOriginalReleaseAndDistributor)).toBe('(Original Release Date; Studio/Distributor), ');
  });

  it('should generate note citation format with only city and video release date', () => {
    expect(filmInfoNote(onlyCityAndVideoRelease)).toBe('(City: Video Release Date), ');
  });
...
describe('FilmInfo.filmBibliography() test', () => {
  it('should generate bibliography citation format', () => {
    expect(filmInfoBibliography(filmInfo)).toBe('Original Release Date; City: Studio/Distributor, Video Release Date. ');
  });

  it('should generate bibliography citation format with no original author specified', () => {
    expect(filmInfoBibliography(noOriginalRelease)).toBe('City: Studio/Distributor, Video Release Date. ');
  });

  it('should generate bibliography citation with no city specified', () => {
    expect(filmInfoBibliography(noCity)).toBe('Original Release Date; Studio/Distributor, Video Release Date. ');
  });

  it('should generate bibliography citation with no studio/distributor specified', () => {
    expect(filmInfoBibliography(noDistributor)).toBe('Original Release Date; City: Video Release Date. ');
  });

  it('should generate note citation with no video release date specified', () => {
    expect(filmInfoBibliography(noVideoRelease)).toBe('Original Release Date; City: Studio/Distributor. ');
  });

  it('should generate note citation with only original release specified', () => {
    expect(filmInfoBibliography(onlyOriginalRelease)).toBe('Original Release Date. ');
  });
  ...
  it('should generate note citation with only original release date and city specified', () => {
    expect(filmInfoBibliography(onlyOriginalReleaseAndCity)).toBe('Original Release Date; City. ');
  });

  it('should generate note citation with only distributor and video release date specified', () => {
    expect(filmInfoBibliography(onlyDistributorAndVideoRelease)).toBe('Studio/Distributor, Video Release Date. ');
  });

  it('should generate note citation with only only original release date and distributor specified', () => {
    expect(filmInfoBibliography(onlyOriginalReleaseAndDistributor)).toBe('Original Release Date; Studio/Distributor. ');
  });
  ...
  it('should generate note citation with only city and video release date specified', () => {
    expect(filmInfoBibliography(onlyCityAndVideoRelease)).toBe('City: Video Release Date. ');
  });

  it('should generate bibliography citation format with equal dates', () => {
    expect(filmInfoBibliography(equalDates)).toBe('Date; City: Studio/Distributor. ');
  });
