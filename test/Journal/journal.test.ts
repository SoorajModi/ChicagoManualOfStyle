...
describe("journalNote() testing", () => {
  test("should create note citation with one author", () => {
    expect(journalNote(journal, { page: "1" })).toBe(
      'First Last, "Title," Publisher vol, no. iss (Year): 1.'
    );
  });

  test("should create note citation with two authors", () => {
    expect(journalNote(twoAuth, { page: "1" })).toBe(
      'Author One and Author Two, "Title," Publisher vol, no. iss (Year): 1.'
    );
  });

  test("should create note citation with three authors", () => {
    expect(journalNote(threeAuth, { page: "1" })).toBe(
      'Author One, Author Two, and Author Three, "Title," Publisher vol, no. iss (Year): 1.'
    );
  });

  test("should create note citation with no authors", () => {
    expect(journalNote(noAuthor, { page: "1" })).toBe(
      '"Title," Publisher vol, no. iss (Year): 1.'
    );
  });

  test("should create bibliography citation for journal with URL", () => {
    expect(journalNote(urlJournal, { page: "1" })).toBe(
      'First Last, "Title," Publisher vol, no. iss (Year): 1, URL.'
    );
  });
});
...
describe("journalBibliography() testing", () => {
  test("should create bibliography citation with one author", () => {
    expect(journalBibliography(journal)).toBe(
      'Last, First. "Title." Publisher vol, no. iss (Year): 1-10.'
    );
  });

  test("should create bibliography citation with two author", () => {
    expect(journalBibliography(twoAuth)).toBe(
      'One, Author, and Author Two. "Title." Publisher vol, no. iss (Year): 1-10.'
    );
  });

  test("should create bibliography citation with three authors", () => {
    expect(journalBibliography(threeAuth)).toBe(
      'One, Author, Author Two, and Author Three. "Title." Publisher vol, no. iss (Year): 1-10.'
    );
  });

  test("should create bibliography citation with no authors", () => {
    expect(journalBibliography(noAuthor)).toBe(
      '"Title." Publisher vol, no. iss (Year): 1-10.'
    );
  });

  test("should create bibliography citation for journal with URL", () => {
    expect(journalBibliography(urlJournal)).toBe(
      'Last, First. "Title." Publisher vol, no. iss (Year): 1-10. URL.'
    );
  });
});
