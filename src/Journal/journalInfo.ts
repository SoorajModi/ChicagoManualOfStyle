import {
  JournalInfoInterface,
  createJournalInfo,
} from "./journalInfoInterface";

export default function journalInfoCitation(
  info: JournalInfoInterface
): string {
  let citation = createJournalInfo(info);
  return noPublishingInfo(citation)
    ? ""
...
function createJournalCitation(
  title: string,
  vol: string,
  iss: string,
  date: string
  ...
  return `${
    titleCitation(title, vol) +
    volumeCitation(vol, iss, date) +
    issueCitation(iss) +
    dateCitation(date)
  }:`;
}
