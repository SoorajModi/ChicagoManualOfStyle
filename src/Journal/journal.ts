import { JournalInterface, createJournal, validJournalInterface } from './journalInterface';
...
export function journalNote(info: JournalInterface, page: {page: string}): string {
  ...
  if (citation.url !== '') {
    return (`${(standardJournalNote(citation, page)).replace(/.$/, ', ') + citation.url}.`);
  }

  return standardJournalNote(citation, page);
}

function standardJournalNote(citation: validJournalInterface, page: {page: string}): string {
