...
export function filmNote(info: FilmInterface, scene: string): string {
  let citation: validFilmInterface = createFilm(info);
  return `${filmSceneNote(scene) + citation.title}, ${filmNoteDirector(
    citation
  )}${filmInfoNote(citation.info)}${citation.medium}.`;
}

export function filmBibliography(info: FilmInterface): string {
  let citation: validFilmInterface = createFilm(info);
  return `${filmBibDirector(citation) + citation.title}. ${filmInfoBibliography(
    citation.info
  )}${citation.medium}.`;
}
...
function filmNoteDirector(citation: validFilmInterface): string {
  return citation.director.length() === 0
    ? ""
    : `directed by ${citation.director.authors[0].forwardString()} `;
}

function filmBibDirector(citation: validFilmInterface): string {
  return citation.director.length() === 0
    ? ""
    : `${citation.director.authors[0].reverseString()}, dir. `;
}
