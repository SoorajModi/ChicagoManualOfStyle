...
export function filmInfoNote(info: FilmInfoInterface): string {
  return isEmpty(info)
    ? ""
    : `(${addOriginalRelease(info)}${addCity(info)}${addDistributor(
        info
      )}${addVideoReleaseDate(info)}), `;
}
...
function addOriginalRelease(info: FilmInfoInterface): string {
  if (
    info.originalRelease === info.videoRelease &&
    info.city === "" &&
    info.distributor === ""
  )
    return info.originalRelease;
  if (
    (info.city !== "" || info.distributor !== "" || info.videoRelease !== "") &&
    info.originalRelease !== ""
  )
    return `${info.originalRelease}; `;
  return info.originalRelease;
...
function addCity(info: FilmInfoInterface): string {
  return (info.distributor !== "" || info.videoRelease !== "") &&
    info.city !== ""
    ? `${info.city}: `
    : info.city;
}
