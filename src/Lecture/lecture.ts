...
export function lectureNote(info: LectureInterface): string {
  ...
  return `${citation.professors.note()}"${citation.courseTitle}" ${lectureInfoNote(citation.info)}.`;
}

export function lectureBibliography(info: LectureInterface): string {
  let citation: validLectureInterface = createLecture(info);
  return `${citation.professors.bibliography()}"${citation.courseTitle}." ${lectureInfoBibliography(citation.info)}.`;
}
...
function shortNotePrepend(citation: validLectureInterface): string {
  return (citation.professors.length() === 0) ? '' : `${citation.professors.authors[0].last}, `;
}
