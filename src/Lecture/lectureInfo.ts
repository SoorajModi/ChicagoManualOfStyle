...
export function lectureInfoNote(info: LectureInfoInterface): string {
  return isLectureInfoEmpty(info) ? "(lecture)" : createNoteCitation(info);
}

export function lectureInfoBibliography(info: LectureInfoInterface): string {
  return isLectureInfoEmpty(info)
    ? "Lecture"
    : createBibliographyCitation(info);
}

function createNoteCitation(info: LectureInfoInterface): string {
  return `(${citationStart(info, "lecture")}${universityCitation(
    info
  )}${cityCitation(info)}${provinceCitation(info)}${info.date})`;
}

function createBibliographyCitation(info: LectureInfoInterface): string {
  return (
    citationStart(info, "Lecture") +
    universityCitation(info) +
    cityCitation(info) +
    provinceCitation(info) +
    info.date
  );
}
...
function universityCitation(info: LectureInfoInterface): string {
  return info.university === ""
    ? ""
    : info.university + appendUniCitation(info);
}
...
function provinceCitation(info: LectureInfoInterface): string {
  return info.province === ""
    ? ""
    : info.province + appendProvinceCitation(info);
}
