...
export function bookNote(info: BookInterface, page: string): string {
  ...
  if (citation.url !== "") {
    return `${
      standardBookNote(citation, page).replace(/.$/, ", ") + citation.url
    }.`;
  }
...
function standardBookNote(citation: validBookInterface, page: string): string {
  if (citation.editors.length() > 0) {
    if (citation.authors.length() === 0) {
      return `${
        citation.editors.noAuthorsNote() +
        citation.title +
        bookInfoNote(citation.info)
      }, ${page}.`;
    }
    return `${
      citation.authors.note() +
      citation.title +
      citation.editors.editorNote() +
      bookInfoNote(citation.info)
    }, ${page}.`;
  }

  return `${
    citation.authors.note() + citation.title + bookInfoNote(citation.info)
  }, ${page}.`;
}
...
function standardBookBibliography(citation: validBookInterface): string {
  if (citation.editors.length() > 0) {
    if (citation.authors.length() === 0) {
      return `${
        citation.editors.noAuthorBibliography() + citation.title
      }.${bookInfoBibliography(citation.info)}`;
    }
    return `${
      citation.authors.bibliography() + citation.title
    }.${citation.editors.editorBibliography(
      citation.info.edition || ""
    )}${bookInfoBibliography(citation.info)}`;
  }
