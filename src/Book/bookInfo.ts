import {
  BookInfoInterface,
  ValidBookInfoInterface,
  createBookInfo,
} from "./bookInfoInterface";

export function bookInfoNote(info: BookInfoInterface): string {
  let citation: ValidBookInfoInterface = createBookInfo(info);
  return isBookEmpty(citation)
    ? ""
    : `${bookEditionNote(citation.edition)}${addSpaceBetweenElements(
        citation
      )}${createBookCitation(citation, true)}`;
}
...
export function bookInfoBibliography(info: BookInfoInterface): string {
  ...
  return isBookEmpty(citation)
    ? ""
    : `${bookEditionBibliography(citation.edition)}${addSpaceBetweenElements(
        citation
      )}${createBookCitation(citation, false)}`;
}

function isBookEmpty(info: ValidBookInfoInterface): boolean {
  return (
    info.edition === "" && info.pub === "" && info.pop === "" && info.yop === ""
  );
}
...
function createBookCitation(
  citation: ValidBookInfoInterface,
  note: boolean
): string {
  if (isMiddleElementsEmpty(citation)) return "";

  if (note) {
    return `(${generateMiddleElements(
      citation.pub,
      citation.pop,
      citation.yop
    )})`;
...
function generateMiddleElements(
  pub: string,
  pop: string,
  year: string
): string {
